...
- ✅ **Cancellation acceptance from the container client** (2026-07-08): a second `test_macos` run interrupted via Esc in Claude Code ~4 minutes in. Client side: clean in-flight cancellation (no transport error, no partial result). Host side: `notifications/cancelled` → `Server._oncancel` → AbortError; the `xcodebuild` process group died within seconds on SIGTERM alone (no SIGKILL escalation), zero orphaned processes, session and server unaffected — the exact orphan scenario recorded under "Layer A2 confirmed" in the validation findings.
